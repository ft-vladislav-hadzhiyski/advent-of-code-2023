...
var task1Numbers = lines
    .Select(line => line.ToCharArray().Where(ch => char.IsNumber(ch)).ToList())
    .Select(lineNums => new[] { int.Parse(lineNums[0].ToString()), int.Parse(lineNums[^1].ToString()) })
    .Select(pair => pair[0] * 10 + pair[1])
    .ToList();

Console.WriteLine("task 1 = {0}", task1Numbers.Sum());
...
var task2Numbers = lines.Select(rawLine =>
{
    ...
    for (var i = 0; i < chars.Length; i += 1)
    {
        ...
        var numberWords = digits.Keys;
        foreach (var numberWord in numberWords)
        {
            var wordLength = numberWord.Length;
            if (i <= chars.Length - wordLength)
            {
                var word = line.Substring(i, wordLength);
                if (word == numberWord)
                {
                    numbers.Add(digits[numberWord]);
                    continue;
                }
            }
        }
    ...
    return numbers;
}).Select(pair => pair[0] * 10 + pair[^1]).ToList();

Console.WriteLine("task 2 = {0}", task2Numbers.Sum());
