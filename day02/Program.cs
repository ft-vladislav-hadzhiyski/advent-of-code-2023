var input = @"Game 1: 1 blue, 2 green, 3 red; 7 red, 8 green; 1 green, 2 red, 1 blue; 2 green, 3 red, 1 blue; 8 green, 1 blue
...
var games = input.Split(new[] { '\r', '\n' }, StringSplitOptions.RemoveEmptyEntries).Select(line =>
{
    var id = line.Split(new[] { ' ', ':' })[1];
    var sets = line
    .Split(new[] { ':' })[1]
    .Split(new[] { ';' })
    .Select(set =>
        ...
        .Select(pair =>
            ...
            }).GroupBy(set => set.Color).Select(groupedSet => new
            {
                Color = groupedSet.Key,
                Count = groupedSet.Select(gs => gs.Count).Sum()
            }).ToList());

    return new
    {
        Id = int.Parse(id),
        Sets = sets.SelectMany(s => s).ToList()
    };
}).ToList();
...
var task1 = games
    .Where(game =>
        game.Sets.All(s =>
            (s.Color == "red" && s.Count <= 12) ||
            (s.Color == "green" && s.Count <= 13) ||
            (s.Color == "blue" && s.Count <= 14)))
    .ToList();

Console.WriteLine("Task 1 = {0}", task1.Select(g => g.Id).Sum());
...
.Select(game =>
    game.Sets
        ...
        {
            Color = pair.Key,
            Max = pair.Select(p => p.Count).Max()
        })
        .ToList()
)
.Select(t =>
    t.Select(p => p.Max)
    .Aggregate((res, cr) => res * cr));

Console.WriteLine("Task 2 = {0}", task2.Sum());
