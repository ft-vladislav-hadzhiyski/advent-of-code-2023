...
var input = @"48.................501....33.....622..............763.........331.................161.683......................................980..........
...
// find all symbols - not a number, dot, \r or \n
var symbols = new HashSet<char>(input.ToCharArray()).Where(ch => !char.IsNumber(ch) && ch != '.' && ch != '\r' && ch != '\n').ToList();

var lines = input.Split(new[] { '\r', '\n' }, StringSplitOptions.RemoveEmptyEntries);
var adjacentLines = lines.Select((line, index) =>
{
    var adjacents = new List<StringValue>();
    if (index > 0)
    {
        adjacents.Add(new StringValue(lines[index - 1], index - 1));
    }
...
var numbersAndSymbols = adjacentLines.Select(line =>
{
    var ns = NumbersAndSymbolsFinder.Find(line.Line, symbols);

    return new
    {
        line.Line, // for debugging
        LineIndex = line.Index, // for debugging
        ns.Numbers,
        ns.Symbols,
        Adjacent = line.Adjacent.Select(adjacent => NumbersAndSymbolsFinder.Find(adjacent.Value, symbols)).ToList(),
    };
}).ToList();

var parts = numbersAndSymbols
    .SelectMany(ns => ns.Numbers
                    .Where(num =>
                        // number has a symbol next to it
                        ns.Symbols.Any(s => s.Index == num.Index - 1 || s.Index == num.Index + num.Length) ||
                        // number has a symbol touching on an adjacent line
                        ns.Adjacent.Any(adj => adj.Symbols.Any(adjs => adjs.Index >= num.Index - 1 && adjs.Index <= num.Index + num.Length)))
                    .ToList())
    .ToList();
var partsSum = parts.Sum(part => part.Value);

Console.WriteLine("Task 1 = {0}", parts.Sum(p => p.Value));

var linesWithPartNumbers = adjacentLines.Select(line =>
{
    var ns = NumbersAndSymbolsFinder.Find(line.Line, symbols);
    var adjacent = line.Adjacent.Select(adj =>
    {
        var adjacents = new List<StringValue>();
        if (adj.Index > 0)
        {
            adjacents.Add(new StringValue(lines[adj.Index - 1], adj.Index - 1));
        }
        if (adj.Index < lines.Length - 1)
        {
            adjacents.Add(new StringValue(lines[adj.Index + 1], adj.Index + 1));
        };

        var adjAdjacent = adjacents.Select(aadj => NumbersAndSymbolsFinder.Find(aadj.Value, symbols));

        var adjns = NumbersAndSymbolsFinder.Find(adj.Value, symbols);
        return new
        {
            Line = adj.Value,
            adj.Index,
            adjns.Numbers,
            adjns.Symbols,
            PartNumbers = adjns.Numbers
            .Where(num =>
                // number has a symbol next to it
                ns.Symbols.Any(s => s.Index == num.Index - 1 || s.Index == num.Index + num.Length) ||
                // number has a symbol touching on an adjacent line
                adjAdjacent.Any(adj => adj.Symbols.Any(adjs => adjs.Index >= num.Index - 1 && adjs.Index <= num.Index + num.Length)))
            .ToList()
        };
    }).ToList();
    ...
    return new
    {
        ...
        ns.Symbols,
        Adjacent = adjacent,
        PartNumbers = ns.Numbers
            .Where(num =>
                // number has a symbol next to it
                ns.Symbols.Any(s => s.Index == num.Index - 1 || s.Index == num.Index + num.Length) ||
                // number has a symbol touching on an adjacent line
                adjacent.Any(adj => adj.Symbols.Any(adjs => adjs.Index >= num.Index - 1 && adjs.Index <= num.Index + num.Length)))
            .ToList()
    };
}).ToList();

var gears = linesWithPartNumbers
.SelectMany(lwpn => lwpn.Symbols
    .Where(s => s.Value == "*")
    .Select(s => new
    {
        AdjacentPartsOnThisLine = lwpn.PartNumbers
            .Where(n => s.Index == n.Index - 1 || s.Index == n.Index + n.Length)
            .ToList(),
        AdjacentPartsOnAdjacentLines = lwpn.Adjacent
            .SelectMany(adj => adj.PartNumbers
                .Where(adjn => s.Index >= adjn.Index - 1 && s.Index <= adjn.Index + adjn.Length))
            .ToList()
    }))
.Where(g => g.AdjacentPartsOnThisLine.Count + g.AdjacentPartsOnAdjacentLines.Count == 2)
.Select(g => g.AdjacentPartsOnThisLine.Concat(g.AdjacentPartsOnAdjacentLines).ToList())
.ToList();

var gearRatios = gears.Select(g => g.Aggregate((res, n) => new NumberValue(res.Value * n.Value, 0, 0))).ToList();

Console.WriteLine("Task 2 = {0}", gearRatios.Sum(gr => gr.Value));
