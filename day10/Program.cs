...
var input = @"F|7.F|-F|777FJ7FF77-F777F7F|7..FF-77FF|--JJFJ7-J-F77.FF-F7F-F-FL7|--7---F.FL-7-7J.FFF7|-FFJ7.7.LF-LLJ77|F7FJJ-JFF|-.|FFF7F7..FFL7F-FJ-7-LL77
...
var mapDirections = lines
    .Select((line, row) =>
        line
        .ToCharArray()
        .Select((ch, col) =>
            new MapEntry(new Position(row, col), directions[ch]))
        .ToArray())
    .ToArray();

var map = new Map(mapDirections);
var start = map.Start;

var potentialLoopLines = map.Directions
    .Where((line, index) =>
        index == start.Position.Row - 1 ||
        index == start.Position.Row ||
        index == start.Position.Row + 1)
    .SelectMany(directions =>
        directions.Where((dir, index) =>
            index == start.Position.Col - 1 ||
            index == start.Position.Col ||
            index == start.Position.Col + 1))
    .Where(entry => entry.Direction != Direction.Start && entry.Direction != Direction.Ground)
    .ToList();

var loopStarts = potentialLoopLines.Where(pll => map.CanMove(pll, start));
var paths = new List<Path>();

foreach (var loopStart in loopStarts)
{
    var path = new Path(loopStart);
    var current = loopStart;


    while (current.Direction != Direction.Start)
    {
        current.Visited = true;
        path.Entries.Add(current);
        var next = map.Directions
        .Where((line, index) =>
            index == current.Position.Row - 1 ||
            index == current.Position.Row ||
            index == current.Position.Row + 1)
        .SelectMany(directions =>
            directions.Where((dir, index) =>
                index == current.Position.Col - 1 ||
                index == current.Position.Col ||
                index == current.Position.Col + 1))
        .Where(e => !e.Visited)
        .Where(e => map.CanMove(current, e))
        .SingleOrDefault();
    ...
    map.Directions.SelectMany(d => d).Where(e => e.Direction != Direction.Start).ToList().ForEach(e => e.Visited = false);
}

var distance = 1;
for (var i = 0; i < paths[0].Entries.Count; i += 1)
{
    var p1 = paths[0].Entries[i];
    var p2 = paths[1].Entries[i];
...
Console.WriteLine("Task 1 = {0}", distance);

{
    var height = map.Directions.GetLength(0);
    var width = map.Directions[0].Length;
    var path = paths[0].Entries;
    ...
    for (var i = 0; i < height; i += 1)
    {
        ...
        {
            if (path.Contains(map.Directions[i][j]))
            {
                continue;
            }
            ...
            {
                var current = map.Directions[i1][j1];
                if (path.Contains(current) && current.Direction != Direction.NorthEast && current.Direction != Direction.SouthWest)
                {
                    crosses += 1;
                }
    ...
    // 641
    Console.WriteLine("Task 2 = {0}", enclosed);
}
